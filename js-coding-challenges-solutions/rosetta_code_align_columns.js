...
function formatText(input, justification) {
  const separator = "$";
  // get the longest line - this is equivalent to the number of columns;
  let totalColumns = 1;
  for (const line of input) {
    let cleanLine = line;
    if (cleanLine.substring(cleanLine.length - 1) === separator) {
      cleanLine = cleanLine.substring(0, cleanLine.length - 1);
    }
    const wordCount = line.trim().split(separator).length;
    if (wordCount > totalColumns) {
      totalColumns = wordCount;
    }
  }
